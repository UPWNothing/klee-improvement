...
void consume_record(file_entry *ent) {
    ...
    if (ent->type == TYPEA) {
        ...
        printf("lava_131:%d\n",lava_get(131));
        int kbcieiubweuhc1714636915 = printf("fdata = %f\n"+(lava_get(131))*(0x6c6175de==(lava_get(131))||0xde75616c==(lava_get(131))), ent->data.fdata);
...
int main(int argc, char **argv) {
    ...
    //klee_make_symbolic(f,sizeof(*f),"fp");
    file_header head;
    ...
    for (i = 0; i < head.num_recs; i++) {
        ...
        klee_make_symbolic(ent,sizeof(*ent),"entry");
        consume_record(ent);
//        klee_print_expr("ent=",ent);
    }
    return 0;
}
